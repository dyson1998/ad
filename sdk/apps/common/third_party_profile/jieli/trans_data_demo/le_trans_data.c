...
void ble_profile_init(void)
{
    ...
    ble_sm_setup_init(IO_CAPABILITY_NO_INPUT_NO_OUTPUT, SM_AUTHREQ_SECURE_CONNECTION | SM_AUTHREQ_BONDING, 7, TCFG_BLE_SECURITY_EN);
#if !TRANS_ANCS_EN && !SMART_TCFG_BLE_SECURITY_EN
    gatt_client_init();
    gatt_client_check_ios_config(2);
#endif
#elif PASSKEY_ENTER_ENABLE
...
u8 link_key_data2[16] = {0x11, 0x22, 0x33}; 
void ble_send_test(void)
{
    log_info("ble_send_test\n");
    app_send_user_data(ATT_CHARACTERISTIC_ae02_01_VALUE_HANDLE, link_key_data2, 3, ATT_OP_AUTO_READ_CCC);
}
...
{
    log_info("***** ble_init***111111111111***\n");

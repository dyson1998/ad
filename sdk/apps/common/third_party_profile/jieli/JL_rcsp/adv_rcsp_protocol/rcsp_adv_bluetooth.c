...
enum func_type {
    ADV_BT_FUNCTION      = 0,
    ADV_MUSIC_FUNCTION   = 1,
    ADV_RTC_FUNCTION     = 2,
    ADV_LINEIN_FUNCTION  = 3,
    ADV_FM_FUNCTION      = 4,
    ADV_LIGHT_FUNCTION   = 5,
    ADV_FMTX_FUNCTION    = 6,
};
...
#if TCFG_DEV_MANAGER_ENABLE
    case MSG_JL_FILE_BROWSER_END:
        printf("MSG_JL_FILE_BROWSER_END\n");
        u8 reason = rcsp->args[0];
        adv_app_browser_stop();
        printf("reason = %d\n", reason);
        if (2 == reason) {
#if 0
            // 播歌逻辑用户自己实现
        ...
        adv_app_file_browser_stop(reason);
        break;
#endif
...
{
    u8 current_mode = APP_BT_TASK;
    switch (app_mode) {
    case APP_BT_TASK:
        current_mode = ADV_BT_FUNCTION;
        break;
    case APP_MUSIC_TASK:
        current_mode = ADV_MUSIC_FUNCTION;
        break;
    }
    return current_mode;
}
...
static u32 JL_opcode_get_target_info(void *priv, u8 OpCode, u8 OpCode_SN, u8 *data, u16 len)
{
    ...
    if (mask & BIT(ATTR_TYPE_FUNCTION_INFO)) {
        ...
        u8 function_info = 0;
        u32 func_mask = BIT(0);
        u8 cur_mode = get_cur_mode(app_get_curr_task());
        ...
        memcpy(tmp_buf + 4, &cur_mode, 1);
        memcpy(tmp_buf + 5, &function_info, 1);
...
static u16 common_function_info(u32 mask, u8 *buf, u16 len)
{
    ...
    if (mask & BIT(COMMON_INFO_ATTR_DEV)) {
        ...
        if (dev_manager_find_spec("sd0", 0)) {
            printf("dev [sd0] online\n");
            dev_info.status |= BIT(ADV_APP_BS_SD0);
            dev_info.sd0_handle |= app_htonl((u32)ADV_APP_BS_SD0);
        }
        offset += add_one_attr(buf, len, offset, COMMON_INFO_ATTR_DEV, (u8 *)&dev_info, sizeof(dev_info));
    }
#endif
...
static void JL_rcsp_cmd_resp(void *priv, u8 OpCode, u8 OpCode_SN, u8 *data, u16 len)
{
    ...
    case JL_OPCODE_FILE_BROWSE_REQUEST_START:
        if (false == adv_app_browser_busy()) {
            JL_CMD_response_send(OpCode, JL_PRO_STATUS_FAIL, OpCode_SN, NULL, 0);
        } else {
            JL_CMD_response_send(OpCode, JL_PRO_STATUS_SUCCESS, OpCode_SN, NULL, 0);
            adv_app_browser_start(data, len);
        }
        break;
#endif
...
extern void online_api_spp_recieve_cbk(void *priv, u8 *buf, u16 len);
extern void online_api_spp_state_cbk(u8 state);
extern void online_api_spp_send_wakeup(void);

...
void bt_updata_fix_rx_result()
{
    ...
    printf("sum_corrcet_pkt:%d,sum_err_pkt:%d,err_b:%d sum_b:%d perr:%d herr_b:%d invaile:%d  \n",
           fix_rx_result.rx_correct_p,
           fix_rx_result.rx_err_p,
...
void bt_function_select_init()
{
    ...
    //普通模式下是AAC和SBC是没有区分delay的初始值的，所以上电配置成一样的值。
...
//tx模式设置对应的地址如:
/* u8 fix_rx_mac_addr[6] = {0xE3, 0x7D, 0x2A, 0x87, 0xC4, 0x99 }; */
//rx模式必须设置以下的地址
u8 fix_rx_mac_addr[6] = {0x56, 0x34, 0x12, 0x56, 0x34, 0x12};

/*
 * 对应原来的状态处理函数，连接，电话状态等
 */
void tws_conn_switch_role();
extern void link_fix_rx_dump_result();
extern void fix_rxtx_lt_addr_set(u8 lt_addr);




s32 bt_ble_fre_offset_read_cbk(void *priv);
struct app_offset_t {
    u16 crc;
    s32 offset;
} _GNU_PACKED_;

void fre_offset_write_handle(s32 fre)
{
    struct app_offset_t fre_write;
    /* if(fre) { */
    fre_write.offset = fre;
    fre_write.crc = CRC16((u8 *)&fre_write.offset, 4);
    printf("write freq offset to VM : %d\n", fre);
    int err = syscfg_write(CFG_BT_FRE_OFFSET, &fre_write, 6);
    printf("len%d,crc%x offset%d\n ", err, fre_write.crc, fre_write.offset);
    if (err == 6) {
        printf("write succ\n");
    } else {
        printf("write fail");
...
u32 fre;
struct app_offset_t fre_read;
u8 fre_offset_read_handle(s32 *fre)
{
    u8 len = syscfg_read(CFG_BT_FRE_OFFSET, &fre_read, 6);
    printf("read :%d\n", fre_read.offset);

    printf("len:%d,crc :%x offset%d\n ", len, fre_read.crc, fre_read.offset);
    if ((len == 6) && (fre_read.crc == CRC16((u8 *)&fre_read.offset, 4))) {
        *fre = fre_read.offset;
        printf("read ok:%d\n", fre_read.offset);
        ...
        if (hid_conn_depend_on_dev_company) {
            if (bt->value) {
                ...
                if (get_curr_channel_state() == HID_CH) {
                    ...
                    extern const u8 hid_conn_depend_on_dev_company;
                    if (hid_conn_depend_on_dev_company == 1) {
...
#if TCFG_CHARGE_CALIBRATION_ENABLE
        else if ((u32)event->arg == DEVICE_EVENT_CHARGE_CALIBRATION) {
...
//用于iPhone一拖二状态下的来电静音禁止自动拨歌
u8 disauto_music_play = 1;  //配置iphone静音后自动播歌
u8 disauto_music_play_flag = 0;
int filter_out_sbc_data_en_iphone(u8 *addr)
{
    if (get_call_status() == BT_CALL_INCOMING) {
        if (disauto_music_play_flag == 0) {
            disauto_music_play_flag = 1;
            user_send_cmd_prepare(USER_CTRL_AVCTP_OPID_PAUSE, 0, NULL);
            update_multi_bd_status(addr, 4, 0xFF);
        }
        return 1;
    } else {
        disauto_music_play_flag = 0;
    }
    return 0;
}
/*

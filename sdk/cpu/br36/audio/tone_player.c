...
static int tone_audio_res_close(u8 rpt)
{
    ...
    //由tone_file_list_repeat函数打开的最后的文件在这里释放掉
    if (file_dec->file != NULL) {
...
static void tone_dec_event_handler(struct audio_decoder *decoder, int argc, int *argv)
{
    ...
    case AUDIO_DEC_EVENT_ERR:
        ...
        if (!file_dec->list[file_dec->idx]) {
            ...
            //如果不是最后一个文件,判断是否要 repeat
            //上一个 tone_file_list_repeat 函数里打开的文件，放在tone_audio_res_close 函数里关闭
...
int tone_file_dec_start()
{
    ...
    fget_name(file_dec->file, file_name, 15);
    printf("file_name:%s\n", file_name);
...
#if (TCFG_AUDIO_DAC_MIX_ENABLE && (!TCFG_AUDIO_DHA_AND_TONE_MUTEX || TCFG_MIC_EFFECT_ENABLE))

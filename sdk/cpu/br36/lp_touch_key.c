...
void lp_touch_key_init(const struct lp_touch_key_platform_data *config)
{
    ...
    for (u8 ch = 0; ch < LP_CTMU_CHANNEL_SIZE; ch ++) {
        if (__this->config->ch[ch].enable) {
            ...
            P11_M2P_ACCESS(M2P_MASSAGE_CTMU_CH0_CFG0L + ch * 8) = ((ch_sensitivity_table[ch_sensity + ch * 10].cfg0) & 0xFF);
            P11_M2P_ACCESS(M2P_MASSAGE_CTMU_CH0_CFG0H + ch * 8) = (((ch_sensitivity_table[ch_sensity + ch * 10].cfg0) >> 8) & 0xFF);
            P11_M2P_ACCESS(M2P_MASSAGE_CTMU_CH0_CFG1L + ch * 8) = ((ch_sensitivity_table[ch_sensity + ch * 10].cfg1) & 0xFF);
            P11_M2P_ACCESS(M2P_MASSAGE_CTMU_CH0_CFG1H + ch * 8) = (((ch_sensitivity_table[ch_sensity + ch * 10].cfg1) >> 8) & 0xFF);
            P11_M2P_ACCESS(M2P_MASSAGE_CTMU_CH0_CFG2L + ch * 8) = ((ch_sensitivity_table[ch_sensity + ch * 10].cfg2) & 0xFF);
            P11_M2P_ACCESS(M2P_MASSAGE_CTMU_CH0_CFG2H + ch * 8) = (((ch_sensitivity_table[ch_sensity + ch * 10].cfg2) >> 8) & 0xFF);
    ...
    //CTMU 初始化命令
    if (p11_wakeup_query() == 0 || get_charge_online_flag() || (!(M2P_CTMU_CH_WAKEUP_EN)) || (is_ldo5v_wakeup())) {
        log_info("lp touch init by %d_%d_%d_%d\n", p11_wakeup_query(), get_charge_online_flag(), (!(M2P_CTMU_CH_WAKEUP_EN)), is_ldo5v_wakeup());
